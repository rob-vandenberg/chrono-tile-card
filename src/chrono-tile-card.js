...
//          Without it, Sections view had no declared default size for this
//          card and no basis to offer the native Layout tab, so a fresh
//          instance fell through to ha-card's CSS min-height:100px floor —
//          confirmed via live diagnostic data earlier this session that a
//          real dashboard instance measured exactly 100px tall. Returns
//          rows:6 (≈376px via HA's documented rows×56 + (rows-1)×8 formula,
//          matched to a size confirmed to look right when tested manually
//          via grid_options in YAML), min_rows:2, columns:12 (full width,
//          matching current usage). No max_rows/max_columns — free to grow
//          when resized via the Layout tab.
...
//          state" in item-toggles-row. _renderItem(): <ha-state-icon> now
//          only renders when show_icon !== false.
                ...
                >
                  ...
                  ${isEntity ? html`
                    <div class="item-toggles-row">
                      ${ctToggleField('Show icon', item.show_icon ?? true, e => this._itemToggled(index, 'show_icon', e))}
...
class ChronoTileCard extends LitElement {
  ...
  // Sections-view sizing (instance method, unlike getCardSize — verified via
