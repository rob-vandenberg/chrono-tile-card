  ...
  `;render(){if(!this._config)return html``;const t=this._config;return html`
    ...
    `}}customElements.define("chrono-tile-card-editor",ChronoTileCardEditor);class ChronoTileCard extends LitElement{static properties={_config:{attribute:!1},_itemValues:{state:!0}};static getCardSize(){return 3}getGridOptions(){return{rows:6,min_rows:2,columns:12}}static getConfigElement(){return document.createElement("chrono-tile-card-editor")}static getStubConfig(){return{...DEFAULT_CONFIG,items:[{...DEFAULT_TEMPLATE_ITEM,_id:generateId([]),show:!0,horizontal:"center",vertical:"middle",icon:"",show_state:!1,font_color:"#808080",font_size:12,font_weight:400,line_height:1.2,border_radius:50,background_color:"",padding_vertical:10,padding_horizontal:10,margin_top:"",margin_bottom:"",text_shadow_color:"",text_shadow_blur:0,text_shadow_offset_x:0,text_shadow_offset_y:0,text_shadow_stroke_width:0,text_shadow_layers:2,template:"{{ now().strftime('%H:%M') }}"}]}}constructor(){super(),this._config=null,this._hass=null,this._itemValues={},this._itemSubs=new Map,this._subscribed=!1,this._holdTimer=null,this._holdFired=!1,this._lastTapTime=0,this._lastTapTarget=null,this._pendingTapTimer=null,this._touchStartX=null,this._touchStartY=null,this._pressedItem=null}set hass(t){const e=this._hass;this._hass=t,this._config&&this._setupSubscriptions(),this._hassShouldRender(e,t)&&this.requestUpdate()}get hass(){return this._hass}_hassShouldRender(t,e){if(!t||!e)return!0;const i=this._config;if(!i)return!0;if(t.locale!==e.locale||t.formatEntityState!==e.formatEntityState)return!0;const o=new Set;i.dimmer_entity&&o.add(i.dimmer_entity);for(const t of i.items??[])t.entity&&o.add(t.entity);for(const i of o)if(t.states?.[i]!==e.states?.[i])return!0;return!1}setConfig(t){({config:t}=migrateConfig(t)),this._config=t,this._hass&&this._setupSubscriptions()}connectedCallback(){super.connectedCallback(),this._hass&&this._config&&!this._subscribed&&this._setupSubscriptions(),this.style.setProperty("--editor-preview-aspect-ratio",isInsideEditDialog(this)?"16 / 10":"auto"),this.style.setProperty("--editor-preview-height",isInsideEditDialog(this)?"auto":"100%")}disconnectedCallback(){super.disconnectedCallback(),this._teardownSubscriptions()}_setupSubscriptions(){this._itemSubs||(this._itemSubs=new Map);const t=this._config?.items??[],e=this._config??{},i=new Set,o=t=>t.replace(/_([a-z])/g,(t,e)=>e.toUpperCase()),n={},r=new Set(["zone_alignment","items"]);for(const t of Object.keys(DEFAULT_CONFIG)){if(r.has(t))continue;const i="card"+o(t).replace(/^./,t=>t.toUpperCase());n[i]=e[t]??DEFAULT_CONFIG[t]}n.cardDimmerOpacity=Math.round(1e3*this._computeDimmerOpacity())/10,t.forEach((t,e)=>{if(!("template"in t))return;const o=`item-${e}`;i.add(o);const{text:r,fullyLiteral:a}=substituteCardVariables(t.template??"",n),s=this._itemSubs.get(o);if(s&&s.substituted===r)return;if(s?.unsub&&this._unsubscribeOne(s.unsub),a)return this._itemValues={...this._itemValues,[o]:r},void this._itemSubs.set(o,{substituted:r,unsub:null});const l=this._hass.connection.subscribeMessage(t=>{this._itemValues={...this._itemValues,[o]:t.result}},{type:"render_template",template:r});this._itemSubs.set(o,{substituted:r,unsub:l})});for(const[t,e]of[...this._itemSubs.entries()])if(!i.has(t)){e.unsub&&this._unsubscribeOne(e.unsub),this._itemSubs.delete(t);const{[t]:i,...o}=this._itemValues;this._itemValues=o}this._subscribed=!0}_unsubscribeOne(t){Promise.resolve(t).then(t=>{"function"==typeof t&&t()}).catch(()=>{})}_teardownSubscriptions(){if(this._itemSubs){for(const t of this._itemSubs.values())t.unsub&&this._unsubscribeOne(t.unsub);this._itemSubs.clear()}this._subscribed=!1}_handleAction(t,e="tap"){this._hass&&handleAction(this,this._hass,t,e)}_resolveActionItem(t){const e=t.target.closest?.("[data-item-index]");if(!e)return null;const i=Number(e.getAttribute("data-item-index"));return this._config.items?.[i]??null}_onPointerDown(t){t.currentTarget.setPointerCapture(t.pointerId),this._pressedItem=this._resolveActionItem(t),this._touchStartX=t.clientX,this._touchStartY=t.clientY,this._holdFired=!1,this._holdTimer&&clearTimeout(this._holdTimer),this._holdTimer=setTimeout(()=>{this._holdFired=!0,this._handleAction(this._pressedItem??this._config,"hold")},500)}_onPointerUp(t){if(null===this._touchStartX)return;const e=t.clientX-this._touchStartX,i=t.clientY-this._touchStartY,o=this._pressedItem;this._touchStartX=null,this._touchStartY=null,this._pressedItem=null,this._holdTimer&&(clearTimeout(this._holdTimer),this._holdTimer=null);const n=Math.abs(e)>=40&&Math.abs(e)>=Math.abs(i),r=Math.abs(i)>=40&&Math.abs(i)>Math.abs(e);if(n)return void(this._holdFired||this._handleAction(this._config,e<0?"swipe_left":"swipe_right"));if(r)return void(this._holdFired||this._handleAction(this._config,i<0?"swipe_up":"swipe_down"));if(this._holdFired)return;const a=o??this._config,s=Date.now();if(s-this._lastTapTime<250&&this._lastTapTarget===a)return this._lastTapTime=0,this._lastTapTarget=null,this._pendingTapTimer&&(clearTimeout(this._pendingTapTimer),this._pendingTapTimer=null),void this._handleAction(a,"double_tap");this._lastTapTime=s,this._lastTapTarget=a,this._pendingTapTimer&&clearTimeout(this._pendingTapTimer),this._pendingTapTimer=setTimeout(()=>{this._pendingTapTimer=null,this._handleAction(a,"tap")},250)}_onPointerCancel(){this._touchStartX=null,this._touchStartY=null,this._pressedItem=null,this._holdTimer&&(clearTimeout(this._holdTimer),this._holdTimer=null)}_onPointerMove(t){if(null===this._touchStartX||!this._holdTimer)return;const e=t.clientX-this._touchStartX,i=t.clientY-this._touchStartY;(Math.abs(e)>=40||Math.abs(i)>=40)&&(clearTimeout(this._holdTimer),this._holdTimer=null)}_computeDimmerOpacity(){const t=this._config,e=t.dimmer_entity??"";if(!e)return 0;const i=this._hass?.states?.[e],o=i?parseFloat(i.state):0;if(isNaN(o))return 0;const n=t.dimmer_lux_min??DEFAULT_CONFIG.dimmer_lux_min,r=t.dimmer_lux_max??DEFAULT_CONFIG.dimmer_lux_max,a=(t.dimmer_opacity_min??DEFAULT_CONFIG.dimmer_opacity_min)/100,s=(t.dimmer_opacity_max??DEFAULT_CONFIG.dimmer_opacity_max)/100,l=t.dimmer_aggressiveness??DEFAULT_CONFIG.dimmer_aggressiveness,c=Math.pow(10,(l-50)/50);if(r<=n)return s;const d=Math.max(0,Math.min(1,(o-n)/(r-n)));return s+(a-s)*Math.pow(d,.33*c)}_itemContainerStyleMap(t){return{"border-radius":pxValue(t.border_radius),"background-color":t.background_color||void 0,"padding-top":pxValue(t.padding_vertical),"padding-bottom":pxValue(t.padding_vertical),"padding-left":pxValue(t.padding_horizontal),"padding-right":pxValue(t.padding_horizontal),"margin-top":pxValue(t.margin_top),"margin-bottom":pxValue(t.margin_bottom)}}_itemTextStyleMap(t){const e=_FONT_OPTION_BY_VALUE.get(t.font_family);return{color:t.font_color||void 0,"font-family":(e?e.family:t.font_family)||void 0,"font-style":e?.italic?"italic":void 0,"font-size":emValue(t.font_size),"font-weight":rawValue(t.font_weight),"line-height":rawValue(t.line_height),"text-shadow":t.text_shadow_color&&""!==t.text_shadow_offset_x&&""!==t.text_shadow_offset_y?Array(Math.max(1,Number(t.text_shadow_layers??2)||1)).fill(`${pxValue(t.text_shadow_offset_x??0)} ${pxValue(t.text_shadow_offset_y??0)} ${pxValue(t.text_shadow_blur??0)} ${t.text_shadow_color}`).join(", "):void 0,"-webkit-text-stroke-width":t.text_shadow_color?pxValue(t.text_shadow_stroke_width??0):void 0,"-webkit-text-stroke-color":t.text_shadow_color||void 0}}_itemIsActionable(t,e){const i=t.tap_action?.action;return(e||!!i)&&"none"!==i}_renderItem(t){if(!1===t.show)return html``;ensureFontLoaded(t.font_family);const e=this._config.items.indexOf(t);if("template"in t){const i=`item-${e}`,o=this._itemValues[i]??"",n=this._itemIsActionable(t,!1);return html`
      ...
      `}if("entity"in t){const i=this._hass?.states?.[t.entity],o=this._itemContainerStyleMap(t),n=this._itemTextStyleMap(t);if(!i)return html`
        ...
        `;const r=t.show_state?t.attribute?`${t.prefix??""}${i.attributes?.[t.attribute]??""}${t.suffix??""}`:this._hass?.formatEntityState?this._hass.formatEntityState(i):i.state:"",a=this._itemIsActionable(t,!0);return html`
        <div
          class="overlay-entity-item${a?" clickable":""}"
          data-item-index="${e}"
          style=${styleMap(o)}
